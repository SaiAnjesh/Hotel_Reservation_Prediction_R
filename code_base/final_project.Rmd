...
We see that there are only 61 records in Mar 1 2018 and 165 Records in Feb 28. So, I am gonna update the Feb 29 value
Let us further inspact the date column.

```{r}
# Understading the date
date_check_data <- data.frame(
  Date = seq(as.Date("2017-07-01"), as.Date("2018-12-31"), by = "day")
)
str(dates_orig_data)
str(date_check_data)
date_check_data <- merge(x = date_check_data, y = dates_orig_data, by = "Date", all.x = T)
sum(is.na(date_check_data))
...
if(over_write_models) {
  ...
  set.seed(1)
  # Automatice Tuning / Not included hyper parmater values
...
grid_svm <- expand.grid(C = c(.5, 1, 3, 5))
...
# class(runs)
# MODEL_NAME <- "NEURAL_NETWORK"; MODEL_LIST[[MODEL_NAME]] <- LASSO_MODEL;
# saveRDS(MODEL_LIST[[MODEL_NAME]], file = paste(models_folder_path,"/", MODEL_NAME,"_MODEL.rds",sep = ""))
runs_save_file_name = paste("NN_RUNS_", format(Sys.time(), "%Y%m%d_%H%M"), ".csv", sep = "")
runs_save_file_name = paste(models_folder_path,"/", runs_save_file_name, sep = "")
...
confusionMatrix(predicted.labels, as.factor(test_DV), mode="everything", positive="1")
```
